"""
Test script for Google ADK + Couchbase MCP Server agent on `gemini-2.5-flash-lite`.
Mirrors the 5 questions from the tutorial notebook so we can A/B compare against
test_flash_agent.py.

Usage:
    pip install google-adk python-dotenv
    python test_flash_lite_agent.py
"""
...
async def main():
    ...
    session = await session_service.create_session(
        app_name="couchbase_agent", user_id="test_flash_lite_user"
    )
    runner = Runner(
        agent=root_agent,
